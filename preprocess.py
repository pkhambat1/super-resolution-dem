...
def get_data(hr_images_filepath, lr_image_width, hr_image_width):
    ...
    for hr_filename in os.listdir(hr_images_filepath):  # TODO: Remove [:50] later
        if hr_filename.split('.')[-1] in {'TIF', 'tiff'}:
    ...
    # Normalize
    hr_images /= 255
    hr_images = tf.squeeze(tf.image.resize(tf.expand_dims(hr_images, axis=3), size=(hr_image_width,hr_image_width)), axis=3) # TODO: remove later
    lr_images = tf.squeeze(tf.image.resize(tf.expand_dims(hr_images, axis=3), size=(lr_image_width,lr_image_width)), axis=3)
