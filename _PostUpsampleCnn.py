...
class PostUpsampleCnn(tf.keras.Model):
    '''
    ...
    def __init__(self, lr_image_width, hr_image_width):
        ...
        """
        super(PostUpsampleCnn, self).__init__()
        ...
        class Up(tf.keras.Model):
            ...
            def __init__(self, filters, kernel_size, dropout=False, strides=2):
                ...
                self.model1 = keras.Sequential(
                    [
                        Conv2DTranspose(filters, kernel_size, strides=strides, **self.conv_args),
                        Dropout(.2),
                    ]
                )
        ...
        # downsampling
        self.d1 = Down(512, (3, 3), False)
        self.d2 = Down(512, (3, 3), False)
        ...
        self.u4 = Up(512, (3, 3), False)
        self.u5 = Up(3, (3, 3), False)
        self.scale_up = Conv2DTranspose(3, (3, 3), strides=self.upscale_factor)  # look into
    ...
    def call(self, inputs):
        ...
        u5 = self.u5(u4)
        # u5 = tf.concat([u5], -1)
        scale_up = self.scale_up(u5)
        plt.imshow(u5[0]*255)
        plt.show()
        # out = self.out(scale_up)
        return scale_up
